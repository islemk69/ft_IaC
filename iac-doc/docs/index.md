...
Infrastructure-as-code is a paradigm where provisionning and deployement is done declaratively.

Instead of dozens of commands writed manually by a user, scripts running commands step by step or a deployer creating instances one by one in the cloud provider web interface everything is declared inside a (or multiple) file.
...
For this project we choose Terraform, for it is the most common IaC tool used in professionnal world, and Google Cloud because it's free offer is very good for beginning and experimentation.
...
- The deployement region: `gcp_region`
- The instance type: `machine_type`
- An email address for alerting: `alert_email`

Region possibles values: `US`, `Paris`, `Frankfurt`
...
- `terraform.tfstate{.backup}`: state file descriving the stack state 
...
Pratically, it is used only to destroy the compute instance :
```bash
